...
def heuristic(initial_position, current_position, visited_points, total_checkpoints):
  return total_checkpoints - len(visited_points) + dist(initial_position, current_position)
...
def astar(nodes, initial_position, total_checkpoints):
  ...
  heappush(open_set, (heuristic(initial_position, initial_position, visited_check_points, total_checkpoints), next(counter), initial_position, visited_check_points))

  while open_set:
    _, _, position, visited_check_points = heappop(open_set)

    if heuristic(initial_position, position, visited_check_points, total_checkpoints) == 0:
    ...
    for next_step in neighbors:
      ...
      if not next_state in g_scores or \
        ...
        f_score = tentative_g_score + heuristic(initial_position, position, next_visited_check_points, total_checkpoints)
        heappush(open_set, (f_score, next(counter), next_position, next_visited_check_points))

  return False

test_params = read_input("test_input.dat")
assert astar(*test_params) == 20
