  ...
  while len(queue) > 0:
    ...
    for move in available_moves(passcode, current_path, current_position):
      ...
      if current_position == (3, 3):
        if len(current_min_path) == 0 or len(current_min_path) > len(current_path):
          current_min_path = current_path
      elif len(current_min_path) == 0 or len(current_min_path) > len(current_path):
        queue.append([current_path, current_position])

  return current_min_path

assert brute_force_search("hijkl") == ""
assert brute_force_search("ihgpwlah") == "DDRRRD"
assert brute_force_search("kglvqrro") == "DDUDRLRRUDRD"
assert brute_force_search("ulqzkmiv") == "DRURDRUDDLLDLUURRDULRLDUUDDDRR"
